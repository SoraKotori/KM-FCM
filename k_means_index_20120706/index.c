...
double	DSI(int k,struct center k_center[],struct data dataset[],struct config conf[])
{
	/****變數宣告*****/
	int		i,j;
	double	distance;
	...
	for(j = 0;j < k; j++)
	{
		for(i = 0;i < conf[0].datasize; i++)
		{
			if(dataset[i].cid	==	j)
			{
				distance	=	point_distance(dataset[i].point,k_center[j].point,conf);
				sum[j]		=	sum[j]+distance;
			}
		}
		normal	=	normal+sum[j];
	}
	//normal	=	1.0/((normal/k)+1.0);
	normal=normal/k;
	return normal;
}
